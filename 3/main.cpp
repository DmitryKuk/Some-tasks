// Ниже представлен пример полной программы.
// Опции могут быть как встроенными типами, так и произвольными (легко обобщается).
// Непосредственно разбором argv занимается функция option_set<...>::parse,
// всё остальное -- структура данных для обработки параметров.
// Для компиляции требуется C++14.
